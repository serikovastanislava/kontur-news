...
export default function SideNavFeed({ mode }) {
  const { t, lang, user, favorites, userArticles, openModal } = useApp();
  ...
  let items = [...userItems, ...staticItems];

  if (mode === 'Популярное') {
    items = items.filter(a => getViews(a.id) > 100);
  } else if (mode === 'Последние') {
    items = items.filter(a => a.publishedAt ? (Date.now() - a.publishedAt) / 3_600_000 <= 2 : approxHoursAgo(a.time) <= 2);
